...
document.querySelectorAll("[data-manager]").forEach(link => link.addEventListener("click", async event => {
  ...
  try {
    if(type==="branding"){const settings=await api("/api/branding");content.innerHTML=`<form class="quick-form" id="brandingForm"><label><span>Akzentfarbe</span><input name="accent" type="color" value="${settings.accent||"#1ca471"}"></label><label><span>Logo (PNG/JPG, max. 500 KB)</span><input name="logo_file" type="file" accept="image/png,image/jpeg"></label><label><span>Favicon (PNG/JPG/ICO, max. 500 KB)</span><input name="favicon_file" type="file" accept="image/png,image/jpeg,image/x-icon"></label><button class="button primary">Design speichern</button></form><p>Leere Dateifelder behalten die bisher gespeicherten Bilder bei.</p>`;document.querySelector("#brandingForm").addEventListener("submit",async e=>{e.preventDefault();const form=e.currentTarget;try{const logo=await readImage(form.logo_file);const favicon=await readImage(form.favicon_file);const payload={accent:form.accent.value,logo:logo||settings.logo||"",favicon:favicon||settings.favicon||""};await api("/api/branding",{method:"POST",body:JSON.stringify(payload)});applyBranding(payload);managerDialog.close();toast("Design gespeichert","Logo, Favicon und Farbe sind jetzt aktiv.")}catch(error){toast("Design nicht gespeichert",error.message)}});return}
