...
if [ -n "$existing_project" ]; then
  ...
  for checked_port in 80 443 8080; do
...
if has ufw && $AS_ROOT ufw status 2>/dev/null | grep -q 'Status: active'; then
  warn "UFW is active. Allow TCP 80/443 and every configured stream port manually."
elif has firewall-cmd && $AS_ROOT firewall-cmd --state >/dev/null 2>&1; then
  warn "firewalld is active. Allow TCP 80/443 and every configured stream port manually."
fi
...
if [ "$NO_START" -eq 0 ]; then
  ...
  while [ "$attempt" -le 30 ]; do
    if curl -fsS --max-time 2 http://127.0.0.1:8080/ >/dev/null 2>&1; then ready=1; break; fi
    sleep 2
    attempt=$((attempt + 1))
  done
  if [ "$ready" -ne 1 ]; then
